# Geely EX2 – Apps & Installation Guide

> 🌐 [Tiếng Việt](README.md)

The Geely EX2 app suite consists of 3 files for 3 different devices:

| File | Used for | Download |
|---|---|---|
| `GeelyEX2.ipa` | iPhone (iOS) | [⬇️ Download](GeelyEX2.ipa) |
| `Geely-Ex2-Phone.apk` | Android phone | [⬇️ Download](Geely-Ex2-Phone.apk) |
| `Geely-Ex2-Car.apk` | Geely EX2 head unit | [⬇️ Download](Geely-Ex2-Car.apk) |

## 📱 Installation Guides

> 👉 **Jump right in:**

- [**Install on iOS (iPhone)**](INSTALL-IOS.en.md)
- [**Install on Android (phone + head unit)**](INSTALL-ANDROID.en.md)

### Tips

- Installing the app on your phone (iOS or Android) is simple — just follow the matching guide.
- Installing the app on the **head unit** requires unlocking ADB first — follow Part 2 of the Android guide carefully and read it fully before you start.

## ☕️ Donate

If you drink Highlands Coffee, grab a voucher before you order.
It takes a few seconds and saves you a fair bit. 😄

[![Highlands Coffee Voucher](https://img.shields.io/badge/☕_Lấy_Voucher_Highlands_Coffee-C8922A?style=for-the-badge&logoColor=white)](https://trackecom.asia/9AHYxyQ3)

👉 https://trackecom.asia/GqBsVT8z

Thanks for the support — consider it a coffee on you! ❤️

Every donation is more motivation to keep building free apps and tools for the community. 🚀
